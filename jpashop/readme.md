...
여기서는 itemService.saveItem(book)에서 수정을 시도하는 Book 객체다. Book객체는 이미 DB에 한번 저장되어서 식별자가 존재한다. 이렇게 임의로 만들어낸 엔티티도 기존 식별자를 가지고 있으면 준영속성 엔티티로 볼 수 있다.
...
> 트랜잭션 안에서 엔티티를 다시 조회, 변경할 값 선택 -> 커밋 시점에 dirty checking이 동작해서 DB에 update SQL 실행
...
3. 조회한 영속 엔티티(DB)에 엔티티의 값(변경할 값)을 채워넣는다. 
4. 
